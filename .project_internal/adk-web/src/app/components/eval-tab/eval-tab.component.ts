...
export class EvalTabComponent implements OnInit, OnChanges {
  ...
  private timestampCache: Map<string|number, string> = new Map();
  ...
  ngOnInit(): void {
    // Initialize eval sets when component loads
    if (this.appName() !== '') {
      this.getEvalSet();
      this.getEvaluationResult();
    }
      ...
      this.evalService.getEvalSets(this.appName())
          ...
          .subscribe((sets) => {
            if (sets !== null) {
              ...
              this.evalsets = sets;
              // Use markForCheck instead of detectChanges to avoid cycles
  ...
  openNewEvalSetDialog() {
    ...
    dialogRef.afterClosed().subscribe((needRefresh) => {
      if (needRefresh) {
        ...
        this.cachedSortedHistory = [];
        this.cachedSortedHistoryKey = '';
      }
    });
  }
  ...
  listEvalCases() {
    ...
    this.evalService.listEvalCases(this.appName(), this.selectedEvalSet)
        .subscribe((res) => {
          ...
          // Clear cache when listing cases
          this.cachedSortedHistory = [];
          this.cachedSortedHistoryKey = '';
        });
  }
  ...
  runEval() {
    ...
    this.evalService
        ...
        .subscribe((res) => {
          ...
          this.getEvaluationResult();
          // Clear cache when evaluation completes
          this.cachedSortedHistory = [];
          this.cachedSortedHistoryKey = '';
        });
  }
  ...
  private cachedSortedHistory: any[] = [];
  private cachedSortedHistoryKey: string = '';

  protected getEvalHistoryOfCurrentSetSorted(): any[] {
    const cacheKey = `${this.appName()}_${this.selectedEvalSet}`;
    
    // Return cached result if available
    if (this.cachedSortedHistoryKey === cacheKey && this.cachedSortedHistory.length > 0) {
      return this.cachedSortedHistory;
    }
    ...
    const evalHistorySortedArray = evalHistorySorted.map((key) => {
      // Pre-compute formatted timestamp to avoid calling formatTimestamp in template
      const formattedTimestamp = this.formatTimestamp(key);
      return {
        timestamp: key,
        formattedTimestamp: formattedTimestamp,
        evaluationResults: evalHistory[key]
      };
    });

    // Cache the result
    this.cachedSortedHistory = evalHistorySortedArray;
    this.cachedSortedHistoryKey = cacheKey;
  ...
  protected formatTimestamp(timestamp: number|string): string {
    // Check cache first to avoid change detection issues
    ...
    if (isNaN(numericTimestamp)) {
      const errorMsg = 'Invalid timestamp provided';
      this.timestampCache.set(timestamp, errorMsg);
      return errorMsg;
    }
    ...
    if (isNaN(date.getTime())) {
      const errorMsg = 'Invalid date created from timestamp';
      this.timestampCache.set(timestamp, errorMsg);
      return errorMsg;
    }
          ...
          for (const evalResultId of res) {
            this.evalService.getEvalResult(this.appName(), evalResultId)
                .subscribe((res) => {
                  ...
                  this.appEvaluationResults[this.appName()][res.evalSetId][timeStamp] =
                      uiEvaluationResult;
                  // Clear cache when evaluation results are updated
                  this.cachedSortedHistory = [];
                  this.cachedSortedHistoryKey = '';
                });
          }
